                    ...
                    latestNews.slice(0, 1).map((data, index) =>
                        <div className='blog_card_in reveal' key={index}>
                            <Link className="blog_card_link block" to={`${data.link}`}>
                                ...
                                <div className={`cursor-pointer type uppercase text-right text-[#2b2b2b94]  tracking-[1px]`}> {data.date} | {data.type}</div>
                                {/* ${data.name?.length > 100 ? 'lg:mt-[-18px] mt-[-20px]' : 'lg:mt-[5px]'} */}
                            </Link>
                        </div>
                    )
                )
