                    ...
                    latestNews.slice(0, 1).map((data, index) =>
                        <div className='blog_card_in reveal' key={index}>
                            <Link className="blog_card_link block" to={`${data.link}`}>
                                ...
                                <p className='name mt-5 text-[#0061AB] xl:text-[17px] text-[14px] font-light lg:leading-[29px] leading-[25px]'> {data.name?.length > 100 ? `${data.name.slice(0, 90)}...` : data.name} </p>
                                <div className={`type uppercase text-right text-[#2b2b2b94] ${data.name?.length > 100 ? 'lg:mt-[-18px] mt-[-20px]' : 'lg:mt-[5px]'} tracking-[1px]`}> {data.date} | {data.type}</div>
                            </Link>
                        </div>
                    )
                )
