...
const Career = () => {
  const {
    ...
    error: pageDataError,
  } = useFetchData("page-sections", "5");

  // 🔹 Extract Banner Data Safely
  const extractPageData = (pageData) => {
    if (!pageData)
      return {
        banner: { image: "", heading: "Default Heading" },
        workCulture: {},
      };
  ...
  if (pageDataError)
    return (
      <p className="text-red-500">Error loading Banner: {pageDataError}</p>
    );
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="keywords"
          content="Great Value Realty careers, real estate jobs, property industry careers, real estate career opportunities, job openings at Great Value Realty"
        />
        <meta
          name="description"
          content="Explore exciting career opportunities at Great Value Realty. Join our team of real estate experts and build a successful career in the dynamic property industry. Apply today!"
        />
        <meta
        ...
        <meta
          name="author"
          content="Careers at Great Value Realty | Join Our Real Estate Team"
        />
      ...
      >
        <div className="headingWrap lg:max-w-[79%] max-w-[100%] m-auto text-center">
          <CommonHeading
            HeadingText={"Build Your Future, Shape the Skyline, Grow with Us"}
