...
const [isSitebarFormButtonLoading, setIsSitebarFormButtonLoading] = useState(false);
    ...
    const updateStatusHandler = async (id, selectedStatus) => {
        ...
        var response = await Request('admin/projectdata/floor-plan/'+id+'/status', 'POST', formData);
    ...
    const addSubmitHandler =async (e)=>{
        ...
        var response=await Request('admin/projectdata/floor-plan','POST',formData);
        if (response.status && response.statusCode === 403) {
             toast.error(response.message);
    ...
    const updateSubmitHandler=async (e)=>{
        ...
        var response=await Request('admin/projectdata/floor-plan/'+editId+'/update','POST',formData);
        if (response.status && response.statusCode === 403) {
            setErrors(response.errors);
        }else if(response.status && response.statusCode == 200){
            getlist();
    ...
    const editHandler = async (id)=>{
        ...
        setShowAddSidebar(true)
            ...
            var response=await Request('admin/projectdata/floor-plan/'+id+'/edit','GET');
    ...
    const deleteHandler=async(id)=>{
            ...
            var response=await Request('admin/projectdata/floor-plan/'+id+'/delete','POST');
    ...
    const getSubtypologyhandle=async(typologie_id)=>{
        ...
        const response=await Request('admin/get-all-subtyplogy-by-typology/'+typologie_id,'GET');
        if (response.status && response.statusCode === 200) {
    ...
    const getlist=async ()=>{
        ...
        var response=await Request('admin/projectdata/floor-plan?project_id='+projectid,'GET');
        if(response.status && response.statusCode==200){
    ...
    return(
        <>
            ...
            <div className="property_wizard_listing d-flex">
                        ...
                        (list ? list && list.map(item => (
                            <tr key={item.id}>
                                ...
                                <td>
                                    ...
                                    {(item.image ? <img src={CONFIG.VITE_APP_STORAGE+item.image } alt="" className="img-fluid" /> :"No Image " ) }
                                    </div>
                                </td>

                               
                                <td>
                                     {(item.size ? item.size : "On Request")}
                                </td>
                                <td>
                ...
                <div className="px_50 form_col">
                    <Button className="btn btn_outline" onClick={backHandler}>Back</Button>
                    <Sections projectid={projectid}  section_type={section_id} sub_heading title="Floor Plans"   />
                    <div className="card card_style1 mt_40">
                    <button className="btn ms-auto btn_primary btn-sm" onClick={addAmenityHandler}>Add Floor Plans</button>
                        <div className="d-flex">
                            <h5>All Floor Plans</h5>
                        </div>
                        ...
                        <SideModal onCancel={cancelHandler}  onSubmit={(enableEdit ? updateSubmitHandler : addSubmitHandler)} isLoading={isSitebarFormButtonLoading}>
                            <Form  >
                                    <div className="radio_design mb_20">
                                    {
                                        (subTypologyList ? subTypologyList && subTypologyList.map((item,key) => (
                                            <CustomRadio
                                                checked={sectionFormdata.sub_typology == item.sub_typologies_id}  
                            ...
                            <Form.Group as={Col} md="12" className="mb_20">
                                ...
                                <InputGroup className="mb-3">
                                    <Form.Control placeholder="Enter floor size" value={sectionFormdata.size} name="size" onChange={handleSectionChange}  />
                                    <select className="form-control" name="size_type" id="" onChange={handleSectionChange}  >
                                        <option value="">Price Type</option>
                                        {
                                            Object.entries(sizeListType).map(([key, value]) => {
                                                       return  <option value={key} key={key} selected={sectionFormdata.size_type == key ? true : false} >{value}</option>
                                            })   
                                        }
                                    </select>
                            ...
                            <Form.Group as={Col} md="12" className="mb_20">
                                ...
                                <InputGroup className="mb-3">
                                    <Form.Control placeholder="Enter floor price"  name="price" type="number" value={sectionFormdata.price} onChange={handleSectionChange}  />
                                    {errors.price}
                                </InputGroup>
                                  {sectionFormdata.price ? numDifferentiation(sectionFormdata.price) :null}
                            </Form.Group>
