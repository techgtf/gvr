...
const PlatterBanners = () => {
  ...
  const addSubmitHandler = async (e) => {
    ...
    var response = await Request(
      `admin/platter/${platterid}/banner`,
      "POST",
      formData
    );
    if (response.status && response.statusCode === 403) {
  ...
  const updateSubmitHandler = async (e) => {
    ...
    formData.append("alt_text", sectionFormdata.alt_text);
    var response = await Request(
      `admin/platter/${platterid}/banner/${editId}/update`,
      "POST",
      formData
    );
    if (response.status && response.statusCode === 403) {
  ...
  const editHandler = async (id) => {
    ...
    var response = await Request(
      `admin/platter/${platterid}/banner/${id}/edit`,
      "GET"
    );
  ...
  const deleteHandler = async (id) => {
    setIsLoading(true);
    var response = await Request(
      "admin/projectdata/floor-plan/" + id,
      "DELETE"
    );
    if (response.status) {
      getlist();
              ...
              <form>
                <div className="grid grid-cols-1 gap-5">
                  <div className="mb-5">
                    ...
                    {sectionFormdata.desktop_image_preview && (
                      <img
                        width="100"
                        src={`${
                          CONFIG.VITE_APP_STORAGE +
                          sectionFormdata.desktop_image_preview
                        }`}
                      />
                    )}
                  ...
                  <div className="mb-5">
                    ...
                    />
                    {errors.mobile_image}
                    {sectionFormdata.mobile_image_preview && (
                      <img
                        width="100"
                        src={`${
                          CONFIG.VITE_APP_STORAGE +
