...
const OurInfrastuchture = ()=>{
    ...
    const addSubmitHandler =async (event)=>{
        ...
        formData.append('value', valueRef.current.value); 

        var response=await Request('admin/infrastructure','POST',formData);
        if(response.status && response.statusCode==403){
            setErrors(response.errors);
            toast.error(response.message);
    ...
    const editHandler = async (id)=>{
        ...
        var response=await Request('admin/infrastructure/'+id,'GET');
        if (response.status && response.statusCode === 200) {
            ...
            valueRef.current.value = response.data.value;
    ...
    const deleteHandler= async (id)=>{
        var response=await Request('admin/infrastructure/'+id,'DELETE');
        if (response.status && response.statusCode === 200) {
            toast.success(response.message);
    ...
    const listHandler = async (search="") => {
        ...
        var response=await Request('admin/infrastructure?search='+search+'&page='+currentPage,'GET');
        if (response.status && response.statusCode === 200) {
    ...
    const updateAmenityHandler=async (event)=>{
        ...
        var response=await Request('admin/infrastructure/'+editId+'/update','POST',formData);
        setIsSitebarFormButtonLoading(false);
                        ...
                        <SideModal onCancel={cancelHandler} onSubmit={(enableEdit ? updateAmenityHandler : addSubmitHandler)} isLoading={isSitebarFormButtonLoading}>
                            <Form >
                                ...
                                <Form.Group>
                                    ...
                                    <Form.Control ref={valueRef} className="" type="text" placeholder="Enter Value" />
                                    {errors.title && <span className="text-danger" >{errors.value}</span>}
...
export default OurInfrastuchture;